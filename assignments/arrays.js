...
console.log(
  `Last car in the inventory is a ${lastCar.car_make} ${lastCar.car_model}`
);
...
for (item of inventory) {
  carModelsLoop.push(item.car_model);
}
let carModelsLoopSorted = [];
carModelsLoopSorted = carModelsLoop.sort();

console.log(carModelsLoopSorted);

let carModels = [];
carModels = inventory.map(obj => {
...
for (item of inventory) {
  carYearsLoop.push(item.car_year);
}

console.log(carYearsLoop);

let carYears = [];
carYears = inventory.map(obj => {
  return obj.car_year;
});
...
for (item of carYears) {
  if (item < 2000) {
    oldCarsLoop.push(item);
  }
}

console.log(oldCarsLoop.length);

let oldCars = [];
oldCars = inventory.filter(obj => obj.car_year < 2000);
...
for (item of inventory) {
  if (item.car_make == "BMW" || item.car_make == "Audi") {
    BMWAndAudinLoop.push(item);
  }
}

console.log(JSON.stringify(BMWAndAudinLoop));

let BMWAndAudi = [];
BMWAndAudi = inventory.filter(
  obj => obj.car_make == "BMW" || obj.car_make == "Audi"
);
